...
- [x] **Product images (partial, real photos)** — real, permissively-licensed stock photos sourced per-noun from Wikimedia Commons (`datasets/products/fetch_stock_photos.py`), hand-reviewed one by one after two rounds of automated filtering still let through a 44% false-positive rate. Covers ~14% of product nouns (15/104); the rest fall back to the existing procedural placeholder. Every photo is attributed on-license via the Glass Mode "PHOTO" badge. See [the dataset card](https://github.com/glasscart/glasscart.github.io/blob/main/datasets/products/DATASET_CARD.md#product-images).
